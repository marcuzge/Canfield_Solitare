...
class Game {
    ...
    /** Undo method, undo the game by one move, until it reaches the initial
     *  state of the game */
    void undo() {
//        System.out.println(_gamestate.size());
        if (_gamestate.size() <= 1) {
            Game temp = _gamestate.get(0);
            this.copyFrom(temp);
//            System.out.println("Cannot undo anymore!");
        } else {
            Game temp = _gamestate.get(_gamestate.size() - 2);
            _gamestate.remove(_gamestate.size() - 1);
            this.copyFrom(temp);
        }
//        } else if (_gamestate.size() == 1 || _gamestate.size() == 2) {
//            System.out.println("Im marcus1");
//            Game temp = _gamestate.get(0);
//            _gamestate.remove(_gamestate.size() - 1);
//            _gamestate.remove(0);
//            this.copyFrom(temp);
...
//            this.copyFrom(temp);
//        }
//    else if (_gamestate.size() == 1) {
//        Game temp = _gamestate.get(0);
//        _gamestate.remove(_gamestate.size());
//        this.copyFrom(temp);
    }
    ...
    /** Store the course of game. */
    public final ArrayList<Game> _gamestate = new ArrayList<>();
